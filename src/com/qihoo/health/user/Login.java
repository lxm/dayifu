...
public class Login {
	...
	public boolean GetCode(String PhoneNum) throws Exception{
		List<NameValuePair> params=new ArrayList<NameValuePair>();
		params.add(new BasicNameValuePair("phone",PhoneNum));
		String ret = this.HttpPost(this.Url + "getcode", params);
		if(ret.contains("OK")){
			return true;
		}
		else{
	...
	public boolean CheckCode(String Code) throws Exception{
		...
		String ret = this.HttpPost(this.Url + "checkcode", params);
		if(ret.contains("OK")){
			return true;
		}
		else{
	...
	private String HttpPost(String Url,List<NameValuePair> params) throws Exception{
		String result = null;
	...
	public String executeHttpPost(String ReqUrl,HashMap<String,String> Param) {
		...
		try {
			...
			while(it.hasNext()){
				...
				dop.writeBytes(entry.getKey() + "=" + entry.getValue());
			}
			dop.flush();
			dop.close();
